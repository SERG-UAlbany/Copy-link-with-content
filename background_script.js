...
(async (browser) => {
  ...
  browser.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (info && info.srcUrl) {
      infoUrl = info.srcUrl;
      infoSelectionText = info.selectionText; 
    }
    startProcessing(tab, info.selectionText);
  });

  const startProcessing = async (tab, selectionText) => {
    try {
    ...
    if (textFragmentURL == false) {        
      await copyToClipboard(infoUrl, infoSelectionText);
    }
  ...
  const copyToClipboard = async (url, selectionText) => {
    ...
    console.log("copytoclip function");
    console.log(url);
    var URLOBJ = new URL(url);
    console.log(selectionText);


    //url = '<div class="kf_reference_copy"> <div class="kf_reference_content"> ' + selectionText.italics()  + "</div>" + ' ' + '<div class="kf_reference_url">' + url + " </div></div>";
